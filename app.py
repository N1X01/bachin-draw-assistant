...
def generate_gcode(name, message):
    gcode = f"""(TechNinja AI Generated G-Code)
...
def process_csv(filepath):
    ...
    with open(filepath, newline='') as csvfile:
        ...
        for row in reader:
            name = row[0]
            message = f"Hey {name}, thank you for your purchase from TechNinja!"
...
def chatbot_response():
    ...
    if "generate gcode" in user_input:
        return jsonify({"response": "🚀 TechNinja: Please upload a CSV file to generate G-code."})
    elif "shopify customers" in user_input:
        return fetch_shopify_customers()
    elif "help" in user_input:
        return jsonify({"response": "🤖 TechNinja AI: I can generate G-code, fetch Shopify customers, or assist you. Try 'Generate G-code' or 'Fetch Shopify Customers'."})
    else:
        return jsonify({"response": "👋 Welcome to TechNinja AI Assistant! Ask me to generate G-code, fetch Shopify customers, or get help."})
